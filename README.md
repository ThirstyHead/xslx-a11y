...
- **SMACSS Theme Engine:** Build-free, text-based CSS theme architecture with 6 bundled palettes (`light`, `dark`, `ocean`, `forest`, `high-contrast`, `print`). Supports custom user themes in `~/.config/xslx-a11y/themes/`.
...
Running `xslx-a11y` via a Python virtual environment is the **primary, recommended path** for all platforms. It works identically on macOS (Apple Silicon & Intel), Windows, and Linux, providing instant access to both the desktop GUI and the headless CLI.
...
git clone https://github.com/ThirstyHead/xslx-a11y.git
cd xslx-a11y
...
- **`examples/Financial-Summary-test.xlsx` (Accessibility Barriers):**
  Intentionally includes digital accessibility barriers checked by `xslx-a11y`: missing title metadata, default "Sheet1" tab name, merged cells disrupting grid navigation, raw tabular data without Table headers, missing chart title/alt text, and low contrast colors.

```bash
# Test the clean workbook (passes cleanly, exits 0):
xslx-a11y "examples/Financial-Summary.xlsx" --format md,html,pdf,json --output-dir ./reports-clean

# Test the barrier workbook (detects barriers, exits 1):
xslx-a11y "examples/Financial-Summary-test.xlsx" --format md,html,pdf,json --output-dir ./reports-barriers
```

Or drag both files directly into the desktop GUI (`xslx-a11y --gui` or `xslx-a11y-gui`) to test batch analysis, progress tracking, and interactive triage!
...
```bash
xslx-a11y workbook.xlsx --format md,html,pdf,json --output-dir ./reports
```
...
```bash
xslx-a11y workbook.xlsx --fix --out-xlsx workbook-remediated.xlsx
```
...
```bash
xslx-a11y workbook.xlsx --triage --out-xlsx workbook-triaged.xlsx
```
...
```bash
xslx-a11y ./spreadsheets --batch --format md,html,pdf,json --output-dir ./batch-reports
```
...
Microsoft Excel includes a built-in accessibility checker (`Review` > `Check Accessibility`). While useful, `xslx-a11y` enforces strict WCAG 2.1 AA criteria that native Office tools often miss or overlook:

1. **Merged Cells:**
   Microsoft Excel allows merged cells if they do not contain formulas. However, screen readers (JAWS, NVDA, VoiceOver) navigate cell grids linearly. Merged cells break coordinate tracking and header associations. `xslx-a11y` unmerges cells and replicates data across the grid so assistive tech never encounters empty fields.
2. **Raw Tabular Ranges:**
   Excel's checker permits rectangular data ranges that visually appear as tables. Without a formal Excel `ListObject` (`Table`), screen readers cannot announce column headers or table boundaries during cell-by-cell navigation. `xslx-a11y` converts rectangular data into structured Tables.
3. **Sheet Naming:**
   Excel often passes default sheet names (`Sheet1`, `Sheet2`) in template or older files. `xslx-a11y` flags default sheet names to ensure screen reader users receive immediate context upon tab switching.
